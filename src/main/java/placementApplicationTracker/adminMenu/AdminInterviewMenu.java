...
public class AdminInterviewMenu {
		...
		while (isRunning) {
			...
			try {
				...
				if (!interviews.isEmpty()) {
					interviews.forEach(interv -> displayInterviewDetails(interv));
				} else {
	...
	private void addInterview(Scanner scanner) {
		try {
	...
	private void editInterview(Scanner scanner) {
		try {
			...
			if (existingInterview != null) {
				...
				Interview updatedInterview = new Interview(interviewId, applicationId, updatedInterviewTimestamp,
						updatedInterviewStatus, updatedInterviewType);
	...
	private void deleteInterview(Scanner scanner) {
		try {
